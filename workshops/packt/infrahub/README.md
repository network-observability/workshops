# Infrahub schema for the Packt workshop
...
nobs packt up           # bring the stack online; wait ~60s for first boot

# Then:
nobs packt load-infrahub
```

`nobs packt load-infrahub` runs `nobs packt schema load infrahub/schema.yml`
...
| `maintenance` | `Boolean`, default `false` | The maintenance gate. Toggled by `nobs packt maintenance --device <name> --state` / `--clear`; the policy in `automation/workshop_sdk.py` skips action when this is true. |
...
calendars, RBAC. For a short workshop the boolean is enough, and it makes
the four canonical paths in Part 3 (quarantine / skip / maintenance-skip /
audit) demoable in one `nobs packt maintenance` call.
...
workshop's loader (`src/packt_workshop/load.py`) reads the same
...
[`../src/packt_workshop/load.py`](../src/packt_workshop/load.py) — find the
...
Then add an `_upsert_ospf_area` helper in `src/packt_workshop/load.py` and call it
...
- It's seeded into the running server via
  **`INFRAHUB_INITIAL_ADMIN_TOKEN`** (see the `infrahub-server` service
  ...
  boot, run `nobs packt destroy && nobs packt up`.
...
| `src/packt_workshop/load.py` | Workshop loader behind `nobs packt load-infrahub`. Idempotent upsert from `lab_vars.yml`; the schema apply step delegates to the shared schema apply in `nobs/commands/schema.py`. |
| `src/packt_workshop/evidence.py` | `nobs packt evidence DEVICE PEER` — Rich panels of the SoT gate, metrics snapshot, log lines, and policy hint for a (device, peer) pair. |
| `../../packages/nobs/src/nobs/clients/infrahub.py` | Generic Infrahub GraphQL client used by `nobs` and the workshop plugins. |
| `../../packages/nobs/src/nobs/commands/schema.py` | Schema apply (wraps `infrahubctl`). Surfaces as `nobs packt schema load PATH`. |
| `../../packages/nobs/src/nobs/commands/maintenance.py` | `nobs packt maintenance` — generic over `--kind`, defaults to `WorkshopDevice`. |
