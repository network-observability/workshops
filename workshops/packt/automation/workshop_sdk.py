"""
Workshop SDK for the Packt hands-on lab.
...
def _build_rca_prompt(device: str, peer_address: str, evidence: dict[str, Any]) -> str:
    return (
        ...
        f"A BgpSessionNotUp alert fired on device {device} for peer {peer_address}.\n"
        "Three evidence sources follow: intent (what the SoT says should be true), "
        "reality (Prometheus metrics, point-in-time), and recent history (Loki log lines).\n\n"
        "EVIDENCE — Source of truth (Infrahub; the intent layer)\n"
        f"{json.dumps(evidence.get('sot', {}), indent=2, default=str)}\n\n"
        "EVIDENCE — Metrics snapshot (Prometheus; the reality layer)\n"
        f"{json.dumps(evidence.get('metrics', {}), indent=2)}\n\n"
        "EVIDENCE — Recent log lines (Loki; BGP-filtered, last 30 min)\n"
        f"{json.dumps(evidence.get('logs', [])[:40], indent=2)}\n\n"
        "TASK\n"
        "Write a root-cause analysis the on-call can read in 60 seconds. Use this exact structure:\n\n"
        "## Severity & confidence\n"
        "- Severity: P1 (customer-impacting), P2 (service-degrading), or P3 (informational / known-broken).\n"
        "- Confidence: high, medium, or low.\n"
        "- One sentence citing the strongest piece of evidence by field name "
        "(e.g. 'oper_state=5 (active) with received_routes=0').\n\n"
        "## Most likely cause\n"
        "2-3 sentences. Ground every claim in a specific evidence field. If the SoT's `reason` field "
        "carries operator context, treat it as a *hint* - verify it against the metrics/logs before "
        "accepting. Surface any contradiction between intent and reality explicitly.\n\n"
        "## Immediate actions\n"
        "3-5 numbered, device-runnable commands. Be specific (`show bgp neighbor 10.1.99.2`, not "
        "'check the config'). Include at least one action that would *disprove* the leading "
        "hypothesis if the cause is something else.\n\n"
        "## What to verify next\n"
        "2-3 bullets covering missing evidence, peer-side signals, and what would shift the "
        "diagnosis. Name absent evidence explicitly rather than inferring around it.\n\n"
        "CONSTRAINTS\n"
        "- Max 1500 characters total.\n"
        "- Use the device name and peer IP throughout.\n"
        "- Do not invent commands, log lines, or metric values that are not in the evidence above.\n"
    )
