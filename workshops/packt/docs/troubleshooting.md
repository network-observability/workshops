...
## `nobs packt up` succeeds but Grafana can't reach Infrahub
...
## `nobs packt load-infrahub` says token mismatch
...
**Recovery.**
```bash
nobs packt destroy   # drops infrahub_db_data
nobs packt up        # re-seeds with the .env token
nobs packt load-infrahub
```
...
The first ~30 seconds after `nobs packt up` are normal — the scenarios register asynchronously.

**Recovery.**
1. Open http://localhost:9090/targets — `telegraf-02` should be `UP`.
2. Run `nobs packt scenarios` — at least one scenario should be registered with `sonda-server`.
3. If `telegraf-02` is `DOWN`, tail it: `nobs packt logs telegraf-02`.
4. If `sonda-server` has no scenarios, tail it: `nobs packt logs sonda-server`.
...
**Recovery.** Be patient (give it 2-3 minutes after `nobs packt up`), then:
```bash
nobs packt alerts            # active alerts in Alertmanager
...
**Recovery.**
1. `nobs packt status` — `Sonda server` should be `ok`. If not, check `nobs packt logs sonda-server`.
...
4. If sonda-server has been restarted, its scenarios are in-memory only — re-kick `sonda-setup`:
   ```bash
   docker compose --project-name packt restart sonda-setup
   ```
...
**Recovery.**
```bash
nobs packt restart prefect-flows   # re-registers the deployment
```
...
- `nobs packt destroy` between sessions to drop volumes (saves several GB of disk too).

## I edited a config / scenario / dashboard and nothing changed

**Why.** Most volume mounts are read-only at container start.
Compose caches the build for services with a `Dockerfile` (webhook, prefect-flows, telegraf-02), and `nobs packt up` defaults to `--no-build` — re-running `up` will reuse the cached image even if you've edited the Dockerfile or the code it copies in.
Grafana provisioning reloads on container restart, not on file change.

**Recovery.**
- Config or YAML change for an existing service: `nobs packt restart`.
- Dockerfile or service-code change: `nobs packt up --build <name>` (or `nobs packt build <name>`).
- `.env` change picked up by an already-built service (e.g. `ENABLE_AI_RCA=true` for `prefect-flows`): `nobs packt restart prefect-flows`.
- Grafana dashboard JSON: `docker compose restart grafana` (or `nobs packt restart`).
