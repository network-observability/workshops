...
Three workshop sites emit one-shot annotations alongside the curriculum data flows: `nobs packt flap-interface` (UPDOWN events), `nobs packt maintenance` (config-push events), and Prefect flows (decision/skip/resolved audit trail). All three route through **sonda-server's `POST /events`** instead of pushing to Loki directly.
...
Workshop CLI (host machine)                       Prefect flows (in-container)
   ┌──────────────────────────┐                      ┌──────────────────────────┐
   │ nobs packt            │                      │ WorkshopSDK.annotate*()  │
...
curl -s http://localhost:8085/health
# Expect: {"status":"ok"}
nobs packt flap-interface --device srl1 --interface ethernet-1/1 --count 2
...
Every other workshop metric flows through the pull path — **sonda emits → Telegraf scrapes `/scenarios/metrics` → Prometheus scrapes Telegraf**. `nobs packt incident` is the exception: its cascade body uses `encoder/sink: remote_write` and pushes directly to Prometheus, bypassing Telegraf. This is intentional, not legacy.
...
- The `--web.enable-remote-write-receiver` flag on Prometheus must stay enabled while `nobs packt incident` exists.
- `incident.py` is the only writer that uses this path. The flag is not load-bearing for anything else.
- If a future PR rewrites the Advanced guide's Act 3 to not depend on `source="incident-cascade"`, this command can be migrated to the pull path and the receiver flag dropped. The header docstring in `src/packt_workshop/incident.py` flags this as a known refactor.
...
- [`docs/preflight.md`](preflight.md) — `nobs packt preflight` regression check.
