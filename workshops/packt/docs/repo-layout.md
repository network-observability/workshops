...
The shape of `workshops/packt/` and what each piece does.
...
workshops/packt/
  ...
  .env.example         # tracked template; copied to .env on first `nobs packt up`
  ...
  vector/              # syslog UDP -> Loki shipper (srl2 log leg)
  infrahub/            # schema YAML + design notes — see infrahub/README.md
  src/packt_workshop/   # workshop-specific commands; registers itself with `nobs`
...
Both are applied by `nobs packt load-infrahub`, which delegates to `src/packt_workshop/load.py`.
...
**"Where do the workshop commands live?"** `src/packt_workshop/` — workshop-specific commands (`load-infrahub`, `evidence`, `try-it`, `flap-interface`, `scenarios`).
The generic ones (`status`, `alerts`, `schema load`, `maintenance`, plus the compose lifecycle) live in [`../../../packages/nobs/`](../../../packages/nobs/) and mount under the `nobs packt` subcommand group, gated by the workshop's declared `capabilities`.
...
- **`nobs`** — the operator toolkit.
  The bare root keeps three workshop-agnostic primitives: `nobs setup`, `nobs preflight`, `nobs workshops`. Workshop ops (`status`, `alerts`, `maintenance`, `schema load`, the docker-compose lifecycle, and any workshop-specific extras) live on each workshop's subgroup, e.g. `nobs packt alerts`. From inside a workshop directory the prefix is optional — `cd workshops/packt && nobs alerts` resolves to the same command via the cwd auto-mount.
  Lives at `packages/nobs/`; the `nobs` console script lands in `.venv/bin/`.

`packt_workshop` is **not** a CLI — it's a Python plugin package that imports `nobs.workshops.register(WORKSHOP)` at module load.
When `nobs.main` imports `packt_workshop`, the plugin registers a `Workshop` instance describing packt's directory, bootstrap hook, capabilities, and extra commands; `nobs` then attaches an `packt` Typer subcommand group with the generic lifecycle (`up`, `down`, `restart`, `ps`, `logs`, `exec`, `build`) plus the workshop-specific commands.
