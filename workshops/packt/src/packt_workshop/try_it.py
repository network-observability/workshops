"""`nobs packt try-it` - guided tour through the four canonical Part 3 paths.
...
def try_it(
    ...
    _wait_for_loki(
        loki,
        '{source="prefect", workflow="packt_quarantine_bgp", device="srl1", peer_address="10.1.99.2", decision="proceed"}',
    ...
    _wait_for_loki(
        loki,
        '{source="prefect", workflow="packt_quarantine_bgp", device="srl1", decision="skip"}',
    ...
    _wait_for_loki(
        loki,
        '{source="prefect", workflow="packt_quarantine_bgp", device="srl1", peer_address="10.1.2.2", decision="skip"}',
    ...
    _wait_for_loki(
        loki,
        '{source="prefect", workflow="packt_quarantine_bgp", device="srl1", decision="resolved"}',
    ...
    console.print(
        Panel.fit(
            ...
            '  [kbd]{workflow="packt_quarantine_bgp"}[/]\n'
...
def _preflight(prom_url: str, loki_url: str, am_url: str, infrahub_url: str) -> None:
    ...
    for name, url in targets:
        try:
            ...
            pass
        fail(f"{name} NOT reachable at {url} - bring the stack up first (nobs packt up)")
        raise typer.Exit(code=1)
    ...
    for path in ("/api/healthcheck", "/health"):
        try:
            ...
            continue
    fail("Infrahub NOT reachable - bring the stack up first (nobs packt up)")
    raise typer.Exit(code=1)
