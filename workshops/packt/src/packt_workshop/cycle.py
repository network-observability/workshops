"""`nobs packt cycle DEVICE PEER` — observe a peer's full Part-3 cycle.
...
def cycle(
    ...
    if not trigger:
        ...
        console.print(f"    [bold]nobs packt cycle {device} {peer} --trigger[/]")
...
def _render_alert_panel(am: AlertmanagerClient, device: str, peer: str) -> None:
    ...
    # Alertmanager's API uses different state names than its UI / `nobs packt
...
def _render_decision_panel(loki: LokiClient, device: str, peer: str, minutes: int) -> None:
    ...
    query = (
        f'{{source="prefect", workflow="packt_quarantine_bgp", '
...
# ---------------------------------------------------------------------------
# Webhook poster — mirrors workshops/packt/src/packt_workshop/try_it.py
# ---------------------------------------------------------------------------
