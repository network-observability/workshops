"""`nobs packt preflight` — run Layer A → B → C, aggregate REPORT.md."""
...
def preflight(
    ...
    Prereqs: stack up (`nobs packt up`), Infrahub seeded
    (`nobs packt load-infrahub`). Layer C requires Playwright:
...
def _render_report(out_dir: Path, rc_a: int, rc_b: int, rc_c: int | None) -> Path:
    ...
    parts: list[str] = [
        f"# Packt preflight — {time.strftime('%Y-%m-%d %H:%M:%S')}\n",
        "## Summary\n",
