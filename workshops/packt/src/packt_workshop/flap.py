"""`nobs packt flap-interface` — inversion-pattern BGP cascade.

The baseline scenarios in `sonda/catalog/srl{1,2}-metrics.yaml` carry
`while: scenario_name=packt-cascade-<device>-<scope>` clauses with
...
One POST per affected scope:
  - Interface flap: `packt-cascade-<device>-intf-<interface-key>`
    (cascade_active signal + oper_state DOWN + UPDOWN log; octets
    freeze via `delay.close.snap_to` on the baseline overrides)
  - Each cascading healthy peer: `packt-cascade-<device>-bgp-<peer-key>`
...
from packt_workshop.flap_topology import Peer, peers_for
...
def interface_cascade_name(device: str, interface: str) -> str:
    ...
    return f"packt-cascade-{device}-intf-{interface.replace('/', '-')}"


def bgp_cascade_name(device: str, peer_address: str) -> str:
    """Deterministic scenario_name the baseline's `while:` references."""
    return f"packt-cascade-{device}-bgp-{peer_address.replace('.', '-')}"
...
def _build_interface_cascade(
    *,
    ...
    return {
        ...
        "category": "network",
        "description": f"Packt inversion cascade — pauses {device}:{interface} baseline during DOWN phase.",
        "defaults": {
            "rate": 1,
            "duration": duration,
...
def _build_bgp_cascade(
    *,
    ...
    return {
        ...
        "category": "network",
        "description": f"Packt inversion cascade — pauses {device} peer {peer.address} baseline during BGP-DOWN phase.",
        "defaults": {
            "rate": 1,
            "duration": duration,
            "labels": {cfg["device_label"]: device},
