"""`nobs packt load-infrahub` - apply schema (via nobs) + seed lab_vars.yml."""
...
# Default paths are anchored to the workshop directory (workshops/packt/).
...
def _wait_for_workshop_schema(client: Any, kinds: list[str], timeout: float, branch: str = "main") -> None:
    ...
    a fresh `nobs packt up` raises `SchemaNotFoundError`.
    """
    import time
