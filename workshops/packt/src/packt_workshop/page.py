"""`nobs packt page` — render a stylized on-call page from a live alert.
...
def page(
    ...
    if not candidates:
        fail("no firing alert matched the filters. Run `nobs packt alerts` to see what's currently firing.")
        raise typer.Exit(code=1)
    ...
    console.print(
        "\n  Confirm the alert: [muted]nobs packt alerts[/]\n  Open the runbook:  [muted]workshops/packt/guides/[/]"
    )
