"""`nobs packt reset` — return the lab to known-good state between runs.
...
def _ensure_infrahub_loaded(infrahub_url: str) -> None:
    """Ensure Infrahub's WorkshopDevice records exist; re-load via `nobs packt load-infrahub` if missing."""
...
def _clear_maintenance_flags() -> None:
    ...
    for device in _WORKSHOP_DEVICES:
        try:
            r = subprocess.run(
                ["uv", "run", "nobs", "packt", "maintenance", "--device", device, "--clear"],
                capture_output=True,
                text=True,
                timeout=30,
                cwd=_repo_root(),
...
def _reapply_sonda_baselines() -> None:
    ...
    try:
        r = subprocess.run(
            ["docker", "compose", "--project-name", "packt", "up", "sonda-setup"],
            capture_output=True,
            text=True,
            timeout=30,
            cwd=_repo_root() / "workshops" / "packt",
        )
        if r.returncode == 0:
...
def _restart_sonda_server_if_cascade_wedged(sonda_url: str) -> None:
    ...
    for scenario in scenarios:
        ...
        if scen_name.startswith("packt-cascade-") or scen_name.startswith("incident-link-failover-"):
            wedged += 1
    ...
    try:
        subprocess.run(
            ["docker", "compose", "--project-name", "packt", "restart", "sonda-server"],
            capture_output=True,
            text=True,
            timeout=30,
            cwd=_repo_root() / "workshops" / "packt",
...
def _restart_sonda_logs_if_wedged(loki_url: str) -> None:
    ...
    try:
        r = subprocess.run(
            ["docker", "compose", "--project-name", "packt", "restart", "sonda-logs"],
            capture_output=True,
            text=True,
            timeout=30,
            cwd=_repo_root() / "workshops" / "packt",
...
def _repo_root():
    ...
    # __file__ = .../workshops/packt/src/packt_workshop/reset.py
