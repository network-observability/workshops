...
| 2 | [Dashboards and Alerts](part-2-dashboards.md) | Mid-morning — a post-mortem email lands. Build the flap-rate panel the team needed yesterday, then walk the same alert from panel to firing to silenced — the threshold and the alert rule are two faces of one decision |
| 3 | [Alert response, Automation and AI](part-3-alerts-automation-ai.md) | Late morning — a real alert fires and your senior narrates how the automation handles it. Walk the four response paths, toggle AI RCA, sign off before lunch |
