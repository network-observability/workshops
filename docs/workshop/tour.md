...
The short version: **if every row says `running` or `unresolved`, the lab is healthy.** A `paused` or `finished` row will only show up during or right after a flap — that's covered in Part 1.
